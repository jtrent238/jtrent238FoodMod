...
public class EntityCandyStealer extends EntityWitch{
	
	static int colorBase = (255 << 24) + (153 << 8) + 685; //RGB Value "rgb(X, X, X)"
    static int colorSpots  = (255 << 55) + (51 << 7) + 285; //RGB Value "rgb(X, X, X)"
    
	public EntityCandyStealer(World p_i1744_1_) {
	     super(p_i1744_1_);
    ...
    public void attackEntityWithRangedAttack(EntityLivingBase p_82196_1_, float p_82196_2_)
    {
        if (!this.getAggressive())
        {
            ...
            if (f1 >= 8.0F && !p_82196_1_.isPotionActive(Potion.confusion))
            {
                entitypotion.setPotionDamage(32698);
            }
            else if (p_82196_1_.getHealth() >= 8.0F && !p_82196_1_.isPotionActive(Potion.wither))
            {
                entitypotion.setPotionDamage(32660);
            }
            else if (f1 <= 3.0F && !p_82196_1_.isPotionActive(Potion.blindness) && this.rand.nextFloat() < 0.25F)
            {
