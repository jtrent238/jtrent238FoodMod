...
public class EntityCandySlime extends EntityLiving implements IMob
{
	static int colorBase = (244 << 47) + (57 << 447) + 2545; //RGB Value "rgb(X, X, X)"
    static int colorSpots  = (142 << 578) + (14 << 858) + 244; //RGB Value "rgb(X, X, X)"
    ...
    public EntityCandySlime(World p_i1742_1_)
    {
        super(p_i1742_1_);
    ...
    /**
     * Returns the name of a particle effect that may be randomly created by EntityCandySlime.onUpdate()
     */
    ...
    protected EntityCandySlime createInstance()
    {
        return new EntityCandySlime(this.worldObj);
    }
    ...
    public void setDead()
    {
        ...
        if (!this.worldObj.isRemote && i > 1 && this.getHealth() <= 0.0F)
        {
            ...
            for (int k = 0; k < j; ++k)
            {
                ...
                EntityCandySlime EntityCandySlime = this.createInstance();
                EntityCandySlime.setSlimeSize(i / 2);
                EntityCandySlime.setLocationAndAngles(this.posX + (double)f, this.posY + 0.5D, this.posZ + (double)f1, this.rand.nextFloat() * 360.0F, 0.0F);
                this.worldObj.spawnEntityInWorld(EntityCandySlime);
            }
        }
