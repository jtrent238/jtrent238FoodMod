...
public class Itemcandyboat extends Item
{
    private static final String __OBFID = "CL_00001774";

    public Itemcandyboat()
    {
        this.maxStackSize = 1;
        ...
        else
        {
            ...
            else
            {
                if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
                {
                    ...
                    EntityCandyBoat entitycandyboat = new EntityCandyBoat(p_77659_2_, (double)((float)i + 0.5F), (double)((float)j + 1.0F), (double)((float)k + 0.5F));
                    entitycandyboat.rotationYaw = (float)(((MathHelper.floor_double((double)(p_77659_3_.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3) - 1) * 90);

                    if (!p_77659_2_.getCollidingBoundingBoxes(entitycandyboat, entitycandyboat.boundingBox.expand(-0.1D, -0.1D, -0.1D)).isEmpty())
                    {
                        return p_77659_1_;
                    }
                    ...
                    {
                        p_77659_2_.spawnEntityInWorld(entitycandyboat);
                    }
