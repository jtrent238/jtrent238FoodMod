...
public class ModBiomes extends BiomeGenBase {
    ...
    public static BiomeGenBase candyland;
	...
	static {
        candyland = (new BiomeCandyLand(97).setColor(5470985).setTemperatureRainfall(0.95F, 0.9F).setHeight(biomeHeight).setBiomeName("CandyLand"));
    ...
    {
        BiomeDictionary.registerBiomeType(candyland);
    ...
    public void genBiomeModdedTerrain(World world, Random random, Block[] replacableBlock, byte[] aByte, int x, int y, double z)
    {
        ...
        for (int l1 = 255; l1 >= 0; --l1)
        {
            ...
            else
            {
                ...
                {
                    if (block2 == BlockLoader.blockcandystone)
                    {
                        if (k == -1)
                        {
                            if (l <= 0)
                            {
                                block = null;
                                b0 = 0;
                                block1 = BlockLoader.blockcandystone;
                            }
                            else if (l1 >= 59 && l1 <= 64)
                            {
                                block = this.topBlock;
                            ...
                            {
                                block = null;
                                block1 = BlockLoader.blockcandygrass;
